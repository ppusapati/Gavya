...
// The gateway forwards /productcatalog.v1. to this service, so a procedure
// addressed under any other prefix never arrives.
func TestServiceNameMatchesTheGatewayPrefix(t *testing.T) {
	if !strings.HasPrefix(ServiceName, "productcatalog.v1.") {
		t.Errorf("ServiceName = %q, want the gateway's productcatalog.v1. prefix", ServiceName)
	}
}

func TestMalformedJSONIsInvalidArgumentNotInternal(t *testing.T) {
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/"+ServiceName+"/GetProduct",
...
func TestGetOnAProcedureIsRejected(t *testing.T) {
	rec := httptest.NewRecorder()
	mux(t).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/"+ServiceName+"/GetProduct", nil))
