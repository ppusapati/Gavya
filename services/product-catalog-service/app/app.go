// Package app is product-catalog-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/product-catalog-service/internal/config"
	"github.com/ppusapati/gavya/services/product-catalog-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "product-catalog-service")
	if err != nil {
		return nil, nil, fmt.Errorf("product-catalog-service: db connect: %w", err)
	}
