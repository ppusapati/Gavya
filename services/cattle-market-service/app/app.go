// Package app is cattle-market-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/cattle-market-service/internal/config"
	"github.com/ppusapati/gavya/services/cattle-market-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "cattle-market-service")
	if err != nil {
		return nil, nil, fmt.Errorf("cattle-market-service: db connect: %w", err)
	}
