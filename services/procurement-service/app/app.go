// Package app is procurement-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/procurement-service/internal/config"
	"github.com/ppusapati/gavya/services/procurement-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "procurement-service")
	if err != nil {
		return nil, nil, fmt.Errorf("procurement-service: db connect: %w", err)
	}
