...
func (r *repo) ListNotifications(ctx context.Context, tenantID, channel, status string) ([]*domain.Notification, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&n.ID, &n.TenantID, &n.RecipientID, &n.RecipientType, &n.Channel,
			&n.Title, &n.Body, &n.Status, &n.Priority, &n.ReferenceID, &n.ReferenceType,
			&n.SentAt, &n.ReadAt, &n.CreatedAt, &n.UpdatedAt, &n.CreatedBy, &n.UpdatedBy, &n.DeletedAt); err != nil {
			return nil, err
		}
...
func (r *repo) ListNotificationTemplates(ctx context.Context, tenantID string) ([]*domain.NotificationTemplate, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.TenantID, &t.EventType, &t.Channel, &t.Title, &t.BodyTemplate,
			&t.IsActive, &t.CreatedAt, &t.UpdatedAt, &t.CreatedBy, &t.UpdatedBy, &t.DeletedAt); err != nil {
			return nil, err
		}
