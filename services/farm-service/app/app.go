// Package app is farm-service's composition root.
//
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "farm-service")
	if err != nil {
		return nil, nil, fmt.Errorf("farm-service: db connect: %w", err)
	}
