// Package app is cattle-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/cattle-service/internal/config"
	"github.com/ppusapati/gavya/services/cattle-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "cattle-service")
	if err != nil {
		return nil, nil, fmt.Errorf("cattle-service: db connect: %w", err)
	}
