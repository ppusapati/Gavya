// Package app is canonical-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/canonical-service/internal/config"
	"github.com/ppusapati/gavya/services/canonical-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "canonical-service")
	if err != nil {
		return nil, nil, fmt.Errorf("canonical-service: db connect: %w", err)
	}
