...
// missing report from an unreachable database. These pin the distinctions.
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err:  fmt.Errorf("load report: %w", repository.ErrNotFound),
			want: connect.CodeNotFound,
		},
		{
			name: "a caller mistake",
			err:  fmt.Errorf("%w: report file not yet available", service.ErrInvalidArgument),
...
func TestClassifyKeepsTheReason(t *testing.T) {
	err := classify(fmt.Errorf("%w: report file not yet available", service.ErrInvalidArgument))

	var connectErr *connect.Error
	if !errors.As(err, &connectErr) {
		t.Fatalf("got %v, want a connect error", err)
	}
	if !strings.Contains(connectErr.Message(), "report file not yet available") {
