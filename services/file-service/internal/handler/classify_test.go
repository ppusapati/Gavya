...
// missing file record from an unreachable database. These pin the distinctions.
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err:  fmt.Errorf("load file record: %w", repository.ErrNotFound),
			want: connect.CodeNotFound,
		},
		{
			name: "a caller mistake",
			err:  fmt.Errorf("%w: file storage path not found", service.ErrInvalidArgument),
...
func TestClassifyKeepsTheReason(t *testing.T) {
	err := classify(fmt.Errorf("%w: file storage path not found", service.ErrInvalidArgument))

	var connectErr *connect.Error
	if !errors.As(err, &connectErr) {
		t.Fatalf("got %v, want a connect error", err)
	}
	if !strings.Contains(connectErr.Message(), "file storage path not found") {
		t.Errorf("message = %q, want it to name the reason", connectErr.Message())
	}
}
