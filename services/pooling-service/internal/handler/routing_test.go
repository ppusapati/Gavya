...
func TestEveryProcedureIsRouted(t *testing.T) {
	procedures := []string{
		...
		"GetEffectiveRetroactivityPolicy",
		"ApplyCorrection",
	}
...
func TestMalformedJSONIsInvalidArgumentNotInternal(t *testing.T) {
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/"+ServiceName+"/GetPool",
...
func TestGetOnAProcedureIsRejected(t *testing.T) {
	rec := httptest.NewRecorder()
	mux(t).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/"+ServiceName+"/GetPool", nil))
