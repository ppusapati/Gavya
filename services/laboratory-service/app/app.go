// Package app is laboratory-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/laboratory-service/internal/config"
	"github.com/ppusapati/gavya/services/laboratory-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "laboratory-service")
	if err != nil {
		return nil, nil, fmt.Errorf("laboratory-service: db connect: %w", err)
	}
