// Package app is ingestion-service's composition root.
//
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "ingestion-service")
	if err != nil {
		return nil, nil, fmt.Errorf("ingestion-service: db connect: %w", err)
	}
