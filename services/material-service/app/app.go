// Package app is material-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/material-service/internal/config"
	"github.com/ppusapati/gavya/services/material-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "material-service")
	if err != nil {
		return nil, nil, fmt.Errorf("material-service: db connect: %w", err)
	}
