...
// missing tenant from an unreachable database. These pin the distinctions.
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err:  fmt.Errorf("load tenant: %w", repository.ErrNotFound),
			want: connect.CodeNotFound,
		},
		{
			name: "a slug already taken",
			err:  repository.ErrDuplicateSlug,
...
func TestClassifyKeepsTheReason(t *testing.T) {
	err := classify(fmt.Errorf("%w: contact_email is required", service.ErrInvalidArgument))

	var connectErr *connect.Error
	if !errors.As(err, &connectErr) {
		t.Fatalf("got %v, want a connect error", err)
	}
	if !strings.Contains(connectErr.Message(), "contact_email is required") {
