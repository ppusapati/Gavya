// Package app is tenant-service's composition root.
//
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "tenant-service")
	if err != nil {
		return nil, nil, fmt.Errorf("tenant-service: db connect: %w", err)
	}
