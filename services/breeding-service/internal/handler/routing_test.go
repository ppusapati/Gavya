...
func TestMalformedJSONIsInvalidArgumentNotInternal(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/"+ServiceName+"/GetBreedingHistory",
...
func TestGetOnAProcedureIsRejected(t *testing.T) {
	rec := httptest.NewRecorder()
	mux(t).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/"+ServiceName+"/GetBreedingHistory", nil))
