...
// missing cycle from an unreachable database. These pin the distinctions.
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err:  fmt.Errorf("load cycle: %w", repository.ErrNotFound),
			want: connect.CodeNotFound,
		},
		{
			name: "a caller mistake",
			err:  fmt.Errorf("%w: cattle_id is required", service.ErrInvalidArgument),
