// Package app is inventory-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/inventory-service/internal/config"
	"github.com/ppusapati/gavya/services/inventory-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "inventory-service")
	if err != nil {
		return nil, nil, fmt.Errorf("inventory-service: db connect: %w", err)
	}
