// Package app is feed-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/feed-service/internal/config"
	"github.com/ppusapati/gavya/services/feed-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "feed-service")
	if err != nil {
		return nil, nil, fmt.Errorf("feed-service: db connect: %w", err)
	}
