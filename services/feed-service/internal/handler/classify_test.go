...
// missing plan from an unreachable database. These pin the distinctions.
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err:  fmt.Errorf("load nutrition plan: %w", repository.ErrNotFound),
			want: connect.CodeNotFound,
		},
		{
...
func TestClassifyKeepsTheReason(t *testing.T) {
	err := classify(fmt.Errorf("%w: cattle_id is required", service.ErrInvalidArgument))

	var connectErr *connect.Error
	if !errors.As(err, &connectErr) {
		t.Fatalf("got %v, want a connect error", err)
	}
	if !strings.Contains(connectErr.Message(), "cattle_id is required") {
