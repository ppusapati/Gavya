...
func TestMalformedJSONIsInvalidArgumentNotInternal(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/"+ServiceName+"/GetAuditLog",
...
func TestGetOnAProcedureIsRejected(t *testing.T) {
	rec := httptest.NewRecorder()
	mux(t).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/"+ServiceName+"/GetAuditLog", nil))
...
// The service is append-only, so nothing may route an update or a delete.
func TestNoMutatingProcedureIsRouted(t *testing.T) {
	m := mux(t)
	for _, p := range []string{"UpdateAuditLog", "DeleteAuditLog"} {
		req := httptest.NewRequest(http.MethodPost, "/"+ServiceName+"/"+p, strings.NewReader(`{}`))
		if _, pattern := m.Handler(req); pattern != "" {
			t.Errorf("%s is routed; audit records are never updated or deleted", p)
		}
	}
}
