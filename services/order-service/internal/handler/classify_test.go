...
// missing order from an unreachable database. These pin the distinctions.
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err:  fmt.Errorf("load order: %w", repository.ErrNotFound),
			want: connect.CodeNotFound,
		},
		{
			name: "an order number already taken",
			err:  repository.ErrDuplicateOrderNumber,
...
// client asked for something the order's state does not allow, and retrying it
// will not help.
func TestBusinessRuleViolationIsInvalidArgument(t *testing.T) {
	err := classify(fmt.Errorf("%w: can only confirm draft orders", service.ErrInvalidArgument))
