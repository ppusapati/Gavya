// Package app is order-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/order-service/internal/config"
	"github.com/ppusapati/gavya/services/order-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "order-service")
	if err != nil {
		return nil, nil, fmt.Errorf("order-service: db connect: %w", err)
	}
