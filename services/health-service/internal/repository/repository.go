...
func (r *repo) ListVaccinationHistory(ctx context.Context, tenantID, cattleID string) ([]*domain.Vaccination, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&v.ID, &v.TenantID, &v.CattleID, &v.VaccineName, &v.BatchNumber,
			&v.AdministeredAt, &v.NextDueDate, &v.VeterinarianID, &v.Dosage,
			&v.CreatedAt, &v.UpdatedAt, &v.CreatedBy, &v.UpdatedBy, &v.DeletedAt); err != nil {
			return nil, err
		}
...
func (r *repo) ListTreatmentHistory(ctx context.Context, tenantID, cattleID string) ([]*domain.Treatment, error) {
	...
	for rows.Next() {
		t := &domain.Treatment{}
		if err := rows.Scan(&t.ID, &t.TenantID, &t.CattleID, &t.DiagnosisCode, &t.Diagnosis,
...
func (r *repo) ListVetVisits(ctx context.Context, tenantID, cattleID string) ([]*domain.VetVisit, error) {
	...
	for rows.Next() {
		v := &domain.VetVisit{}
		if err := rows.Scan(&v.ID, &v.TenantID, &v.CattleID, &v.VeterinarianID, &v.VisitDate,
			&v.Purpose, &v.Notes, &v.Cost, &v.CreatedAt, &v.UpdatedAt, &v.CreatedBy, &v.UpdatedBy, &v.DeletedAt); err != nil {
			return nil, err
		}
		result = append(result, v)
	}
