// Package app is production-service's composition root.
//
...
import (
	...
	"github.com/ppusapati/gavya/services/production-service/internal/config"
	"github.com/ppusapati/gavya/services/production-service/internal/handler"
...
func Build(ctx context.Context, log *p9log.Helper) (serve.Registrar, *pgxpool.Pool, error) {
	cfg := config.Load()
	pool, err := tenantdb.NewPoolFor(ctx, cfg.DatabaseURL, "production-service")
	if err != nil {
		return nil, nil, fmt.Errorf("production-service: db connect: %w", err)
	}
